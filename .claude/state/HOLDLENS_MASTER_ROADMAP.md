...
| 9 | 2 | Sector Rotation Analyzer | `/sectors/` | **shipped v1 (git)** | 2026-04-19 (v1.50) | 2026-04-26 | 2026-05-19 | `/sectors/` hub unifies existing `/rotation` heatmap + `/sector/[slug]` deep-dives. 11 GICS sectors with color-coded cards + method section + schema.org. v2 opportunity: programmatic per-quarter rotation pages. |
| 2 | 1 | SEC Form 4 insider-trading tracker | `/insiders/` | **shipped v1 (git)** | 2026-04-19 (v1.51) | 2026-04-26 | 2026-05-19 | `/insiders/` hub already existed. v1 adds `lib/insider-conviction.ts` (role-weighted conviction scoring, logistic normalization, 10b5-1 penalty) + `/insiders/[insider]/` per-corporate-insider programmatic pages (one per unique tracked insider). v2 will add /ticker/[X]/insiders widget integration + /investor/[X] "how often are insiders buying their picks" overlay. |
